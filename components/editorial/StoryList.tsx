...
export function StoryList({ items, emptyTitle = "Nothing published yet.", empty = "No published stories yet." }: { items: StoryLike[] | SearchResult[]; emptyTitle?: string; empty?: string }) {
  if (!items.length) return <div className="empty-state"><h2>{emptyTitle}</h2><p>{empty}</p></div>;
