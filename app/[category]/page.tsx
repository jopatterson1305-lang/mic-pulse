...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const label = category === "ai" ? "AI" : category[0].toUpperCase() + category.slice(1); const articles = await getPublishedArticles(label);
  return <main className={`content-shell category-shell category-${category}`}><section className="category-intro mic-container"><div><p className="eyebrow">MIC / {label.toUpperCase()}</p><h1 className="display display-lg">{label}<br /><span>in context.</span></h1></div><aside className="category-context liquid-glass"><span className="category-context-index">01 / EDITORIAL VIEW</span><p>{context[category]}</p><span className="category-context-rule" /><span className="category-context-note">Published reporting from the MIC newsroom.</span></aside></section><section className="category-content mic-container"><div className="category-content-label"><span>Latest reporting</span><span>{String(articles.length).padStart(2, "0")} published</span></div><StoryList items={articles} empty="The newsroom is preparing the next signal for this section." /></section></main>;
}
