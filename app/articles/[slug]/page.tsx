...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: `${article.title} — MIC Pulse`, description: article.seo_description ?? article.excerpt ?? "East African business and technology reporting.", alternates: { canonical: `/articles/${article.slug}` }, openGraph: { type: "article", title: article.title, description: article.seo_description ?? article.excerpt ?? "East African business and technology reporting.", url: `/articles/${article.slug}`, images: article.cover_image ? [{ url: article.cover_image }] : undefined, publishedTime: article.published_at ?? undefined }, twitter: { card: "summary_large_image", title: article.title, description: article.excerpt ?? undefined, images: article.cover_image ? [article.cover_image] : undefined } };
}
